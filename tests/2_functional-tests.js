      ...
      test('Convert 10L (valid input)', function(done) {
       chai.request(server)
        ...
        .end(function(err, res){
          ...
          assert.equal(res.body.result.initNum, 10);
          assert.equal(res.body.result.initUnit, 'l');
          assert.approximately(res.body.result.returnNum, 2.64172, 0.1);
          assert.equal(res.body.result.returnUnit, 'gal');
          done();
        });
      });
